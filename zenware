...
Killaura:CreateSlider("Hit Delay", 0.000001, 1, 0.1, nil, 0.01)
...
Reach:CreateSlider("Sword Reach", 1, 28, 14.8, function(val)
	CombatConstant.RAYCAST_SWORD_CHARACTER_DISTANCE = val
end, 0.25)
...
BlockCPSBypass:CreateSlider("CPS", 1, 1000, 20,function(value)
	if loops["blockcps bypass"] then
		cps.BLOCK_PLACE_CPS = value
	end
end)
	...
	if VoidDrop.Values["During Whisper Fly"] == false then
		for i, part in pairs(game.Workspace:GetChildren()) do
			if part.Name == "ServerOwl" then
				if part:GetAttribute("Target") == Player.UserId then
					if part:GetAttribute("Status") == 2 then
						return
					end
				end
			end
