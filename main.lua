...
mainUI.Size = UDim2.new(0.13, 0, 0.5, 0)
...
scrollingFrame.Size = UDim2.new(1, 0, 0.73, 0)
...
local function getESPConfig(obj)
	if obj.Name == "Bee" then
		return Color3.new(1, 1, 0), "Bee"
	elseif obj:FindFirstChild("hidden-metal-prompt") then
		return Color3.new(0, 1, 1), "Metal"
	elseif obj.Name == "CritStar" then
...
local function createESP(obj)
	...
	tracked[obj] = {gui = bill, text = label, part = part, box = box, name = labelName}
end

-- HEARTBEAT (Handles distance and cleanup)
table.insert(connections, RunService.Heartbeat:Connect(function()
	local char = localPlayer.Character
	local hrp = char and char:FindFirstChild("HumanoidRootPart")
	for obj, data in pairs(tracked) do
		if obj and obj.Parent and hrp and data.part then
